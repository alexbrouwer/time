...
class FactoryTest extends TimeTestCase
{
    ...
    public function getValuesForCheckingStringInFormat(): array
    {
        return [
            'valid'                              => ['Y-m-d', '2000-1-1', true],
            'not in format'                      => ['Y-m-d', 'abc', false],
            'non existing date without wrapping' => ['Y-m-d', '2000-04-31', false],
            'non existing date with wrapping'    => ['Y-m-d', '2000-04-31', true, true],
        ];
    }

    /**
     * @dataProvider getValuesForCheckingStringInFormat
     *
     * @param string $format
     * @param string $time
     * @param bool   $expected
     ...
     */
    public function testCanValidateStringIsCorrectFormat($format, $time, $expected, $allowWrapping = false): void
    {
    ...
    public function testTestingAidWithTestNowSet(): void
    {
        ...
        self::assertSame($testDt->format('c'), Factory::getTestNow()->format('c'));
    }
